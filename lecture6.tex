...
\end{theorem}
\begin{proof}
    Let $\{a_n\}$ be our sequence that converges to $a$.  Then, for all $\epsilon > 0$, there exists an $N \in \mathbb{N}$ such that $|a_n -a| < \epsilon$ for all $n \geq N$. Then, $- \epsilon < a_n -a < \epsilon$ implies $-|a| - \epsilon \leq a-\epsilon < a_n$ and $a_n < a + \epsilon \leq |a| + \epsilon $.  This implies that $|a_n| < |a|+ \epsilon$. Consider the set $\{a_1, \;\ldots, a_{N-1}\}$.  Then, for all $n \in \mathbb{N}$
    \begin{align*}
...
Is the converse true?  No, e.g., $\{-1^n\}$.
